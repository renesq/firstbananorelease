...
TEST (rpc, mban_to_raw)
{
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "mban_to_raw");
	request1.put ("amount", "1");
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (rai::kban.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}

TEST (rpc, mban_from_raw)
{
	rai::system system (24000, 1);
	rai::node_init init1;
	auto & node1 (*system.nodes[0]);
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "mban_from_raw");
	request1.put ("amount", rai::kban.convert_to<std::string> ());
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
		system.poll ();
...
TEST (rpc, uban_to_raw)
{
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "uban_to_raw");
	request1.put ("amount", "1");
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
...
TEST (rpc, uban_from_raw)
{
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "uban_from_raw");
	request1.put ("amount", rai::ban_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
