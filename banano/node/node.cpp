...
bool rai::node_config::upgrade_json (unsigned version, boost::property_tree::ptree & tree_a)
{
	...
	switch (version)
	{
		...
		case 3:
			...
			tree_a.put ("receive_minimum", rai::uBAN_ratio.convert_to<std::string> ());
			tree_a.erase ("version");
		...
		if (!rai::read (weight_stream, block_height))
		{
			...
			if (ledger.store.block_count (transaction).sum () < max_blocks)
			{
				...
				while (true)
				{
					...
					BOOST_LOG (log) << "Using bootstrap rep weight: " << account.to_account () << " -> " << weight.format_balance (BAN_ratio, 0, true) << " BANANO";
